...
class Paddlegrab(Powerups):
    ''' defines the paddlegrab power ups for ball'''
    def __init__(self, pad, length, breadth):
        ...
        self.b = 0
        self.alreadyOn = 0
        Powerups.__init__(self, Back.BLACK + Fore.WHITE + "P", Back.BLACK + Fore.WHITE + "G")

    def initials(self ,main):
        for i in range(self.breadth):
            for j in range(self.length):
                if (main['design'].powers[i][j] == 6):
                    self.x[i][j] = i
                    self.y[i][j] = j
        ...
        for i in range(self.breadth):
            for j in range(self.length):
                if (self.status[i][j] == 1):
                    ...
                    main['grid'].change_xy(self.x[i][j] ,self.y[i][j] ,Back.BLACK + Fore.WHITE + "P")
                    main['grid'].change_xy(self.x[i][j] ,self.y[i][j] + 1 ,Back.BLACK + Fore.WHITE + "G")
                    if (self.x[i][j] == main['ball'].max_u - 2):
                        val = main['pad'].y_value
                        l = main['pad'].len - 1
                        diff = int(self.y[i][j]) - val
                        if (val <= int(self.y[i][j]) and diff <= l):
                            main['paddlegrab'].change_val(self.x[i][j] ,self.y[i][j] ,0 ,main, i, j)
                        ...
                        main['paddlegrab'].change_val(self.x[i][j] ,self.y[i][j] ,2, main, i, j)